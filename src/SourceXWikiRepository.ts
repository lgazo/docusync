...
const getWikiSpacePath = (resource: string, source: Record<string, any>) => {
    const pagePart = resource.indexOf(source.url) === 0 ? resource.substring(source.url.length + EXPECTED_PREFIX.length) : resource.substring(EXPECTED_PREFIX.length);
